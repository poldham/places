...
The advantage of this table is that it includes information such as the capital city, the area in square kilometres, the population of the capital city, continent and so on. Some of this data included in the `regions` table (below). However, for general lookup of country names `places_lookup()` will be more flexible and forgiving. 
...
Note that the original featurecode table concatenated the class and feature code, but in the actual country and allcountries files they are separated into `feature_class` and `feature_code`. This makes joining awkward. To solve this a new field called `feature_full` is created at import while dropping the feature_class and feature code fields to prevent duplicates when joining. That is simpler than it sounds. 
...
```{r}
library(ggmap)
kenya_features <- dplyr::left_join(KE, places::featurecodes, by = "feature_full") %>% 
  dplyr::filter(feature_clean == "park headquarters") %>% 
  View()
```
