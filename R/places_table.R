...
#
  ...
  df$is_iso <- NULL
#
#   # option to join to other tables to make selecting countries and regions easier.
  #  if (!is.null(countryname)) {
  #    #load("data/countryinfo.rda")
  #    load("inst/testdata/countryinfo.rda")
  #    countryinfo <- countryinfo %>%
  #      dplyr::select(., iso, iso3, country)
  #    df <- dplyr::left_join(df, countryinfo, by = "iso")
  #  } else {df}
  # if (!is.null(region)) {
  #   #load("data/countryinfo.rda")
  #   load("inst/testdata/countryinfo.rda")
